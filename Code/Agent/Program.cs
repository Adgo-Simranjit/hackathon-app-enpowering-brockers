...
var projectEndpoint = configuration["PROJECT_ENDPOINT"] ?? "https://enpowering-brockers-ai-agent.services.ai.azure.com/api/projects/enpowering-brockers-agent-project";
var modelDeploymentName = configuration["MODEL_DEPLOYMENT_NAME"] ?? "gpt-4.1-mini";
var mcpServerLabel = configuration["MCP_SERVER_LABEL"] ?? "SantoshHackathonAgent";
var mcpServerUrl = configuration["MCP_SERVER_URL"] ?? "https://mymcptest-apavevbrc5gudscm.canadacentral-01.azurewebsites.net/api/mcp";
var userMessage = configuration["USER_MESSAGE"] ?? "Call Exact policy/ Schedule API.";
...
try
{
    ...
    var mcpServerWithKey = $"{mcpServerUrl}";
    ...
    var getPolicyTool = new FunctionToolDefinition(
        name: "GetPolicy",
        description: "Get Policy From Exact.",
        parameters: BinaryData.FromString("""
        {
            ...
            "properties": {
                "policyReference": {
                    "type": "string",
                    "description": "the reference of the policy."
                }
            },
    ...
    var agent = client.Administration.CreateAgent(
        ...
        tools: [getPolicyTool]);//, getSnippetTool, saveSnippetTool]);
    ...
    do
    {
            ...
            && run.Value.RequiredAction is SubmitToolOutputsAction submitToolOutputsAction)
        {
            ...
            foreach (RequiredToolCall toolCall in submitToolOutputsAction.ToolCalls)
            {
                ...
                toolApprovals.Add(new ToolApproval(toolCall.Id, true));
            }

            // Submit the collected tool outputs back to the run
            run = client.Runs.SubmitToolOutputsToRun(thread.Value.Id, run.Value.Id, toolOutputs);
        }
    }
